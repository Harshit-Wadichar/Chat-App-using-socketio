...
const ChatContainer = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      if (typeof sendMessage === "function") await sendMessage({ text: input.trim() });
      setInput("");
    } catch (err) {
      console.error(err);
  ...
  // fetch messages when selectedUser changes (getMessages is memoized)
  useEffect(() => {
    if (selectedUser) {
      getMessages(selectedUser._id);
    } else {
      // clear messages when no user selected (optional)
      // setMessages([]) — but messages state lives in ChatContext, so ChatContext could handle clearing if desired
    }
    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <p className="flex-1 text-lg text-white flex items-center gap-2">
          ...
          {onlineUsers.includes(selectedUser._id) && <span className="w-2 h-2 rounded-full bg-green-500 inline-block ml-1" />}
        </p>
